...
const convertTweetResObjToDbObj = (dbObject) => {
  return {
    tweet: dbObject.tweet,
...
app.post("/login/", async (request, response) => {
  ...
  if (dbUser === undefined) {
    ...
    if (passwordComparing === true) {
      ...
      const jwtToken = jwt.sign(payload, "Nav0727");
...
const twitterUser = async (username) => {
...
app.get("/user/tweets/feed/", authenticateToken, async (request, response) => {
  ...
  const userId = await twitterUser(username);
...
app.get(
  ...
  `;
    ...
    `;
      const getReplyCount = await database.all(getReplyQuery);
      const replyCount = { replies: getReplyCount };
      response.send(replyCount);
    }
  }
);
...
app.get("/user/tweets/", authenticateToken, async (request, response) => {
  ...
  response.send(tweetData.map((tweet) => convertTweetResObjToDbObj(tweet)));
});
